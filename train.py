...
def train(env_name, agent_type='SAC_Normal', episodes=10000, T=1000):
    ...
    if agent_type == 'DDPG_Near':
        ...
        agent = DDPG(env.observation_space.shape[0], env.action_space.shape[0])
    info = f"""
    observation space: {env.observation_space.shape[0]}
    action space: {env.action_space.shape[0]}
    batch size: {agent.batch_size}
    buffer len: {agent.maxlen}
    gamma: {agent.gamma}
    tau: {agent.tau}
...
if __name__ == "__main__":
    ...
    agents = ['SAC_Normal', 'SAC_Near']
    for env in envs:
        for agent_type in agents:
            train(env, agent_type, episodes=2000, T=500)
